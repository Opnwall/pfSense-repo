...
stage_files() {
    ...
    install -m 0644 "${FILES_DIR}${PREFIX}/www/dnscrypt-proxy-querylog.php" \
        "${STAGE_DIR}${PREFIX}/www/"
    install -m 0644 "${FILES_DIR}${PREFIX}/www/dnscrypt-proxy.php" \
        "${STAGE_DIR}${PREFIX}/www/"
...
deploy() {
    ...
    COPYFILE_DISABLE=1 tar --no-xattrs -cf - -C "${STAGE_DIR}" . | \
        ssh "${HOST}" "tar -xf - -C ${REMOTE_DIR}/stage"
...
REMOTE
    ...
    case "${PKG_FILE}" in
        "${REMOTE_DIR}/${PORTNAME}-"*.pkg|"${REMOTE_DIR}/${PORTNAME}-"*.txz)
            ;;
        *)
            echo "Error: Package build did not return a valid package path."
            echo "The SSH account must open a non-interactive shell (not the pfSense console menu)."
            exit 1
            ;;
    esac
