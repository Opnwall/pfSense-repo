...
PKG_NAME="${PKG_NAME:-pfSense-pkg-adguardhome}"
VERSION="${VERSION:-1.0.1}"
ORIGIN="${ORIGIN:-dns/pfSense-pkg-adguardhome}"
COMMENT="${COMMENT:-AdGuard Home DNS filtering integration for pfSense}"
MAINTAINER="${MAINTAINER:-https://github.com/Opnwall/}"
WWW="${WWW:-https://adguard.com/adguard-home/overview.html}"
PREFIX="${PREFIX:-/usr/local}"
FORMAT="${FORMAT:-tgz}"
TARGET_ABI="${TARGET_ABI:-${ABI:-universal}}"
unset ABI || true
OUTPUT_NAME="${OUTPUT_NAME:-${PKG_NAME}.pkg}"
ADGUARDHOME_ASSET="${ADGUARDHOME_ASSET:-AdGuardHome_freebsd_amd64.tar.gz}"
ADGUARDHOME_DOWNLOAD_URL="${ADGUARDHOME_DOWNLOAD_URL:-https://static.adguard.com/adguardhome/release/$ADGUARDHOME_ASSET}"
...
need_file "src/etc/rc.conf.d/adguardhome"
need_file "src/usr/local/etc/rc.d/adguardhome"
need_file "src/usr/local/pkg/adguardhome.xml"
need_file "src/usr/local/pkg/adguardhome.inc"
need_file "src/usr/local/share/pfSense-pkg-adguardhome/info.xml"
need_file "src/usr/local/www/adguardhome.php"
need_file "src/usr/local/www/adguardhome_log.php"
...
extract_adguardhome() {
    archive="$1"
    binary_dst="$2"
    tmpdir="$WORKDIR/adguardhome-extract"

    rm -rf "$tmpdir"
    mkdir -p "$tmpdir"
    tar -xzf "$archive" -C "$tmpdir"

    bin_path="$(find "$tmpdir" -type f -name AdGuardHome | head -n 1)"
    [ -n "$bin_path" ] || die "AdGuardHome binary not found in $archive"

    install -m 0755 "$bin_path" "$binary_dst"
    [ -s "$binary_dst" ] || die "binary is empty: $binary_dst"
}

prepare_binary() {
    local_asset="$SCRIPT_DIR/src/usr/local/bin/$ADGUARDHOME_ASSET"
    archive="$DOWNLOADDIR/$ADGUARDHOME_ASSET"

    if [ -f "$local_asset" ]; then
        echo "==> Using local asset $local_asset"
        cp -f "$local_asset" "$archive"
    else
        echo "==> Downloading $ADGUARDHOME_DOWNLOAD_URL"
        download_file "$ADGUARDHOME_DOWNLOAD_URL" "$archive"
    fi

    extract_adguardhome "$archive" "$STAGEDIR/usr/local/bin/AdGuardHome"
}
...
prepare_binary
rm -f "$STAGEDIR/usr/local/bin/$ADGUARDHOME_ASSET"

install -d -m 0755 "$STAGEDIR/usr/local/etc/adguardhome"
chmod 0644 "$STAGEDIR/etc/rc.conf.d/adguardhome"
chmod 0755 "$STAGEDIR/usr/local/etc/rc.d/adguardhome"
chmod 0755 "$STAGEDIR/usr/local/bin/AdGuardHome"
