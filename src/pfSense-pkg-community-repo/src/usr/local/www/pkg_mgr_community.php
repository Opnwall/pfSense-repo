...
if (isset($_POST['save_community_repo'])) {
	...
	if (!community_repo_write_config($enable_repo)) {
		 ...
		 * Skip that global configuration only for this public repository and
		 * use the system CA bundle; CE accepts the same command. */
		exec('/usr/bin/env SSL_CA_CERT_FILE=/etc/ssl/cert.pem /usr/sbin/pkg -C /dev/null update -f -r ' .
		...
		<form method="post" class="form-horizontal">
			...
			<div class="form-group">
				<div class="col-sm-10 col-sm-offset-2">
					<button type="submit" name="save_community_repo" value="save" class="btn btn-primary"><i class="fa-solid fa-save icon-embed-btn"></i><?=gettext('Save')?></button>
				</div>
			</div>
		</form>
	</div>
...
events.push(function() {
	if (!<?=community_repo_enabled() ? 'true' : 'false'?>) {
		return;
	}
