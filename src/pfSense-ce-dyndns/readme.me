...
开机后会自动执行 `check_install.sh`。如果 pfSense 升级覆盖了相关文件，自愈脚本会检测 `aliyun` 和 `tencentcloud` 是否缺失，缺失时自动重新安装。
...
```sh
./uninstall.sh /root/dyndns-aliyun-tencent-backup-20260621-090000
```
